...
| `BUZZ_PERMESSAGE_DEFLATE_ENABLED` | `false`                   | Offer the `permessage-deflate` WebSocket extension on the relay route. Off by default: each negotiated connection allocates a compressor and a decompressor and holds both for its lifetime. Set `true`/`1`/`on` to enable; junk fails startup. The `buzz_permessage_deflate_enabled` gauge reports the active state. |
